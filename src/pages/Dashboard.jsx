...
/**
 * ======================================================================
 *  ADMIN DASHBOARD — ORIGINAL STYLE + GLOBAL AUTO SCAN
 * ======================================================================
 */
...
const AdminDashboard = () => {
  ...
  const fetchSensorData = useCallback(async () => {
    try {
      ...
      try {
        ...
        console.error("Both ESP32 & Cloud failed:", err);
  ...
  useEffect(() => {
    const fetchInitial = async () => {
      ...
      if (data) setAutoScan({ running: data.status === 1, nextAutoSave: data.next_auto_save_ts });
    };
    fetchInitial();
    ...
    const subscription = supabase
      ...
      .on("UPDATE", payload => {
        setAutoScan({ running: payload.new.status === 1, nextAutoSave: payload.new.next_auto_save_ts });
      })
      .subscribe();
      ...
      <main className="main-content">
        ...
        <section className="scan-controls">
          <div className="button-group">
            ...
            <button className={`start-stop-btn ${autoScan.running ? "stop" : "start"}`} onClick={toggleAutoScan}>
